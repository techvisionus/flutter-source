...
import 'package:flutter_my_page/app/modules/transactions/views/listviewBuilder/exhibition_listview.dart';
...
class ExhibitionMain extends GetView<TransactionsController> {
  bool isReaded = false;
      ...
      child: controller.obx(
        (state) => GetBuilder<TransactionsController>(
          builder: (_dx) => ListViewWithLoadMore(
            ...
            RowEventCallBack: (int index) => buildDataRow(index, _dx, context),
            pullRefreshCallBack: () {},
          ),
        ),
  ...
  InkWell buildDataRow(
    ...
    return InkWell(
      child: ExhibitionListView(_dx?.mListLocalItemBidCar[index]),
      onTap: () {},
    );
  }
